...
### ⚠️ Своя строка сессии на КАЖДОЕ окружение

Одна строка сессии = один auth-key MTProto. Использовать один ключ с двух машин нельзя: Telegram
отзывает его без предупреждения (`406 AUTH_KEY_DUPLICATED`), и оба окружения слепнут разом —
прод перестаёт читать каналы, а `pnpm tg:login` на отозванном ключе даже не доходит до вопроса про
номер. Живой случай 26.07.2026: прод и ноутбук работали с общей строкой, отозвались ВСЕ сессии
аккаунта, восстановление потребовало входа по коду из SMS.

Правильный порядок: один раз войти по номеру (`pnpm tg:login` — эта строка остаётся у разработчика),
а для сервера завести ОТДЕЛЬНУЮ строку без всякого кода:

```bash
pnpm tg:session --out /tmp/prod-session.txt   # подтверждается уже авторизованной сессией
# перенести строку в .env сервера, например:
ssh server 'cat > /root/.s' < /tmp/prod-session.txt   # и вписать в TG_SESSION, файл затем shred
docker compose up -d --force-recreate tg-ingest       # restart НЕ перечитывает .env
```

Тот же принцип для e2e-постера — у него своя строка (`pnpm e2e session` → `E2E_TG_SESSION`).
Итого на аккаунт: строка прода, строка разработчика, строка постера — три разных ключа.
...
3. В админке у каждого канала выставьте осознанные лимиты **до** включения тумблера:
   `trade_size` — **маржа на сделку**, то есть сумма, которая реально уходит с баланса; размер
   позиции получается умножением на плечо (20$ при плече 10 = позиция на 200$). Применяется, когда
   в сигнале не указан риск в процентах. Плюс `max_leverage`, `max_symbol_notional` (потолок
   экспозиции на символ, задаётся в РАЗМЕРЕ ПОЗИЦИИ — главный предохранитель против «риск 2% ×
   большой депозит»), `no_sl_policy`.
...
## 9б. История канала не разбирается (водяной знак)

При подключении канала бот подтягивает его историю целиком — она нужна оператору и бэктесту. Но
**разбирать** её движок не будет: у канала есть водяной знак `channels.process_from_message_id` —
последнее ИСТОРИЧЕСКОЕ сообщение. Разбираются только сообщения СТРОГО новее знака; остальные
получают терминальный статус `archived` без парсера и без вызова AI.

Зачем это нужно, буквально из инцидента 25.07.2026: на новом сервере каналы засеялись с нулевым
курсором, бэкфилл вытянул 3155 сообщений, движок принял их за свежие — **2268 вызовов AI на $22.73,
и сообщение семимесячной давности открыло реальную позицию на mainnet**.

- Знак ставится **один раз**, при первом появлении канала (tg-ingest берёт текущий последний id из
  Telegram). Существующим базам его проставляет миграция 008 = максимальный уже подтянутый id.
- Уже выставленный знак не перезаписывается никогда: иначе рестарт воркера выбрасывал бы
  сообщения, накопившиеся, пока он лежал.
- Проверить: `SELECT id, key, last_seen_message_id, process_from_message_id FROM channels;`

Если журнал нужно обнулить (например, история всё-таки успела разобраться до обновления):

```bash
docker compose exec engine node_modules/.bin/tsx src/reset-journal-cli.ts           # предпросмотр
docker compose exec engine node_modules/.bin/tsx src/reset-journal-cli.ts --force   # применить
```

Команда удаляет действия/сделки/ордера/исполнения/зеркала позиций, переводит подтянутую историю в
`archived` и поднимает водяные знаки. Сообщения и `ai_calls` (потраченные деньги) остаются.
**Отказывается работать при живой позиции или незакрытой сделке** — снести локальный журнал,
оставив позицию на бирже, значит потерять над ней контроль.
