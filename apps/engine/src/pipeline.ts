...
async function resolveBusySymbol(
  ...
  if (position !== undefined || filled) {
    // ПОЗИЦИЯ ЖИВАЯ И ОНА НАША СОБСТВЕННАЯ. symbol_ownership ведётся по паре (канал, символ), и
    // пре-чек в handleOpen ищет владение только ЭТОГО канала — значит, автор повторяет вход в свою
    // же открытую позицию. Это добор: и лимиткой («доливка лимиткой» в лексике автора), и по рынку.
    //
    // Рыночный повтор раньше скипался как «дубль пересланного сигнала». Настоящий дубль — это
    // ПОВТОРНАЯ ОБРАБОТКА ТОГО ЖЕ сообщения, и от неё защищает идемпотентность (дедуп действий по
    // (сообщение, индекс) и orderLinkId), а не этот скип. Живой случай 10.09.2026 (msg 221691
    // «Xrp long с текущих»): накануне автор зафиксировал половину XRP и написал «если вынос вверх
    // дадут то дольюсь» — обещанный добор ушёл в symbol_busy, и мы вошли объёмом вдвое меньше
    // авторского. Цена ошибки в обратную сторону ограничена: добор считается по trade_size —
    // одна стандартная нога, а не копия текущего объёма.
    return { kind: 'add' }
  }
