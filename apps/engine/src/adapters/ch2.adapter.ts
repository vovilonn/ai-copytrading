 ...
 *
 * Признак добора — формулировка «ещё/добираю/доливаю/усредняю» плюс факт открытой позиции по
 * символу. Разбор здесь лишь называет вещи своими именами: даже если формулировки нет и адаптер
 * отдаст `market_entry`, вход по СВОЕЙ живой позиции всё равно станет добором — это решает
 * пайплайн (resolveBusySymbol), у которого есть состояние сделки.
 */
...
/**
 * «Остаток продолжаю удерживать» — про остаток сказано ПРОТИВОПОЛОЖНОЕ закрытию, хотя слово
 * «остаток» и стоит в гейте выхода (живой случай 13.08.2026 в CH1: такая фраза закрыла всю
 * позицию вместо половины; тот же капкан стоял и здесь).
 */
...
function extractCloseOpFromLine(line: string): DeltaOp | null {
  ...
  // «остаток удерживаю» — не выход. Доля, названная в той же строке («зафиксировал 50%, остаток
  // держу»), при этом остаётся: её разбирают ветки процента/слова ниже.
  if (REMAINDER_HELD_RE.test(line) && !/\d{1,3}\s*%|половин|треть|четверт/i.test(line)) return null
