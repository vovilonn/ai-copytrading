...
describe('ch1.adapter — доля/отрицание/полное закрытие фиксации', () => {
  ...
  // Живой случай прода 13.08.2026 (#WLD, msg 3003): «первая цель взята. Зафиксировал 50% позиции,
  // остаток продолжаю удерживать». Зазор между «зафиксировал» и «остаток» перепрыгнул запятую и
  // склеил два предложения в «фиксирую остаток» — бот закрыл ВСЮ позицию вместо половины, и
  // остаток вышел в минус (−2.66) вместо того, чтобы дальше идти к целям автора.
  it('«зафиксировал 50% позиции, остаток продолжаю удерживать» -> доля 0.5, БЕЗ закрытия остатка', () => {
    const result = ops('#WLD - первая цель взята. Зафиксировал 50% позиции, остаток продолжаю удерживать.')

    expect(result).toContainEqual({ op: 'tp_hit' })
    expect(result).toContainEqual({ op: 'partial_close', fraction: 0.5 })
    expect(result).toContainEqual({ op: 'hold' })
    expect(result).not.toContainEqual({ op: 'close_remainder' })
  })

  it('остаток удерживают и без запятой — закрытия всё равно нет', () => {
    for (const text of [
      '#WLD - зафиксировал часть, остаток держу',
      '#WLD - фиксирую половину и остаток продолжаю удерживать',
      '#WLD - остаток оставляю в рынке',
      '#WLD - остаток пока тяну дальше',
    ]) {
      expect(ops(text), text).not.toContainEqual({ op: 'close_remainder' })
    }
  })
