...
export default defineConfig({
  test: {
    ...
    // Дефолтные 5с малы для тестов, гоняющих ПАЧКУ сообщений через весь пайплайн против живой БД
    // (pipeline.e2e/backtest.replay реплеят фикстуру из 100 сообщений: сотни транзакций плюс
    // перехват логов самим vitest). Тест упирался в таймаут не из-за зависания, а просто не
    // помещаясь в него — а красный тест по таймауту читается как сломанный код.
    testTimeout: 30_000,
