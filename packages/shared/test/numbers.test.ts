...
describe('formatWinRate', () => {
  it('decidedTrades=0 -> "—" (нет сделок с известным исходом, не "0%")', () => {
    expect(formatWinRate(0, 0)).toBe('—')
  })

  it('2 победы из 3 решённых -> округление 66.67 -> "67%"', () => {
    expect(formatWinRate(3, 2)).toBe('67%')
  })

  it('0 побед из решённых -> "0%" (известный исход = все проиграны, ЭТО не пустое состояние)', () => {
    expect(formatWinRate(5, 0)).toBe('0%')
  })

  it('все решённые прибыльны -> "100%"', () => {
